...
// File contracts/OffchainTrade.sol
...
contract OffchainTrade is ReentrancyGuard {
    ...
    event OrderListed(address indexed seller , uint256 indexed amount , address indexed token , OrderStatus status);
    ...
    function registerAsBuyer(uint256 _orderId, uint256 _message) external nonReentrant {
        ...
        if (order.status != OrderStatus.Listed) revert AlreadyReleased();
    ...
    function getMessages(uint256 _orderId, address _buyer) public view returns (uint256 _message) {
        if (msg.sender != orders[_orderId].seller) revert NotSeller();
        return orderBuyerMessages[_orderId][_buyer];
    }

    function getOrderDetails(uint256 _orderId) public view returns (Order memory order) {
        return orders[_orderId];
    }

    function getOrderStatus(uint256 _orderId) public view returns (OrderStatus status) {
    Order memory order = orders[_orderId];
    return order.status;
}
